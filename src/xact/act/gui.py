...
class ComputerTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.display_env = os.getenv("WAYLAND_DISPLAY") or os.getenv("DISPLAY")

        if not self.display_env:
            raise ToolError("No display environment detected. Ensure DISPLAY or WAYLAND_DISPLAY is set.")

        # Set screenshot and input tools based on display type
        self.screenshot_cmd = self.detect_screenshot_tool()
        self.xdotool = self.display_env_command("xdotool")
    ...
    def detect_screenshot_tool(self) -> str:
        """Detect and return the screenshot command based on availability."""
        if shutil.which("gnome-screenshot"):
            return f"{self.display_env_command('gnome-screenshot')} -f"
        elif shutil.which("scrot"):
            return f"{self.display_env_command('scrot')} -p"
        elif shutil.which("import"):  # ImageMagick
            return f"{self.display_env_command('import')}"
        else:
            raise ToolError("No compatible screenshot tool found. Install gnome-screenshot, scrot, or import.")

    def display_env_command(self, cmd: str) -> str:
        """Prefix the command with the display environment if necessary."""
        if self.display_env:
            return f"DISPLAY={self.display_env} {cmd}"
        return cmd
    ...
    async def screenshot(self):
        ...
        screenshot_cmd = f"{self.screenshot_cmd} {path}"
